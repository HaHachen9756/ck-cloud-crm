package com.ck.cloud.crm.common.exception.error;
 ...
 * 枚举：错误码
 * <ul>
 *     <li>1~99：用于表示框架或系统内部严重错误，这类错误通常极为罕见且严重影响程序运行</li>
 *     <li>100~999：客户端错误码，当客户端调用出现异常时使用，如参数错误、非法操作等</li>
 *     <li>1000~9999：第三方服务或依赖组件返回的错误，尽管代码本身无误，但因外部因素导致的异常</li>
 *     <li>10000~99999：业务逻辑错误码，这是最常见的错误类型，用于描述业务流程中可能出现的各种正常逻辑处理后的提示信息</li>
 * </ul>
 * <p>
 *     错误码的数值越小，代表错误的严重程度越高，并且预留了足够的空间供后续业务扩展使用（从 20000 开始）
 * </p>
 */
public enum ErrorCode implements ErrorCodeInterface {
    SUCCESS(0, "操作成功", "SUCCESS"), FAILED(99999, "操作失败", "FAILED");
    ...
    public enum Internal implements ErrorCodeInterface {
        ...
        LOGIN_CAPTCHA_GENERATE_FAIL(6, "验证码生成失败", "Internal.LOGIN_CAPTCHA_GENERATE_FAIL"),

        EXCEL_PROCESS_ERROR(7, "Excel 处理失败：{}", "Internal.EXCEL_PROCESS_ERROR"),
        ...
        Internal(int code, String msg, String i18nKey) {
            Assert.isTrue(code < 100,
                "错误码 code 值定义失败，Internal 错误码 code 值范围在 100~999 之间，请查看 ErrorCode.Internal 类，当前错误码码为："
                    + name());

            String errorTypeName = this.getClass().getSimpleName();
            Assert.isTrue(i18nKey != null && i18nKey.startsWith(errorTypeName),
                String.format("错误码 i18nKey 值定义失败，%s 错误码 i18nKey 值必须以 %s 开头，当前错误码为 %s",
                    errorTypeName, errorTypeName, name()));
            this.code = code;
            this.msg = msg;
            this.i18nKey = i18nKey;
        }
    ...
    /**
     * 100~999：是客户端错误码，客户端如（Web、小程序、手机端）调用出错，可能由于参数问题或者授权问题或者调用过于频繁
     */
    public enum Client implements ErrorCodeInterface {
        ...
        Client(int code, String msg, String i18nKey) {
            Assert.isTrue(code > 100 && code < 999,
                "错误码 code 值定义失败，Client 错误码 code 值范围在 100~999 之间，请查看 ErrorCode.Client 类，当前错误码码为："
                    + name());

            String errorTypeName = this.getClass().getSimpleName();
            Assert.isTrue(i18nKey != null && i18nKey.startsWith(errorTypeName), String
                .format("错误码 i18nKey 值定义失败，%s 错误码 i18nKey 值必须以 %s 开头，当前错误码为 %s", errorTypeName,
                    errorTypeName, name()));
            this.code = code;
            this.msg = msg;
            this.i18nKey = i18nKey;
    ...
    public enum External implements ErrorCodeInterface {
        ...
        External(int code, String msg, String i18nKey) {
            Assert.isTrue(code > 1000 && code < 9999,
                "错误码 code 值定义失败，External 错误码 code 值范围在 1000~9999 之间，请查看 ErrorCode.External 类，当前错误码码为："
                    + name());
    ...
    /**
     * 10000~99999：为业务逻辑错误码（无代码异常，代码正常流转，并返回提示给用户）1XX01，XX 是代表类型的意思，比如 10101
     * <ul>
     *     <li>01 的错误码类型为 Permission，错误码的命名最好以类型为开头 比如 NOT_ALLOWED_TO_OPERATE 前面加上 PERMISSION，如：PERMISSION_NOT_ALLOWED_TO_OPERATE</li>
     * </ul>
     */
    public enum Business implements ErrorCodeInterface {
        ...
        MENU_NOT_ALLOWED_TO_CREATE_BUTTON_ON_IFRAME_OR_OUT_LINK(10906, "不允许在 iFrame 和外链跳转类型下创建按钮",
        ...
        Business(int code, String msg, String i18nKey) {
            Assert.isTrue(code > 10000 && code < 99999,
                "错误码 code 值定义失败，Business 错误码 code 值范围在 10000~99999 之间，请查看 ErrorCode.Business 类，当前错误码码为："
                    + name());
